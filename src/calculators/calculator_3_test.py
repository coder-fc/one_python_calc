...
class MockDriverHandlerError:
    def variance(self, numbers: List[float]) -> float:
        return 3  # Representação do que seria o NumpyHandler (elemento)


class MockDriverHandler:
    def variance(self, numbers: List[float]) -> float:
        return 1000000  # Representação do que seria o NumpyHandler (elemento)
...
def test_calculate():
    ...
    assert response == {
        'data': {'Calculator': 3, 'value': 1000000, 'Success': True}}
